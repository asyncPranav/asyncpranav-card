...
const { getProfileViews } = require('./profile-views');
...
async function main() {
  ...
  const profileViews = await getProfileViews(api.USER_NAME);
  ...
  const values = {
    ...
    LANG_LANGUAGES: escapeXml(config.techstack.languages),
    LANG_BACKEND: escapeXml(config.techstack.backend),
    LANG_FRONTEND: escapeXml(config.techstack.frontend),
    LANG_DATABASE: escapeXml(config.techstack.database),
    LANG_TOOLS: escapeXml(config.techstack.tools),
    EMAIL: escapeXml(config.contact.email),
    GITHUB: escapeXml(config.contact.github),
    LINKEDIN: escapeXml(config.contact.linkedin),
    X_HANDLE: escapeXml(config.contact.x),
    ...
    VIEWS_DATA: profileViews != null ? formatNumber(profileViews) : '—',
    PR_DATA: formatNumber(contributions.totalPRs),
    ISSUE_DATA: formatNumber(contributions.totalIssues),
    STREAK_CURRENT: `${streaks.current}d`,
    STREAK_LONGEST: `${streaks.longest}d`,
